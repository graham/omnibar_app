...
var kapture = (function() {
    ...
    // Kapture Stack is what some people will use to make using Kapture easier.
    // While the goal of Kapture is to make it easy to respond to keyboard events,
    // Kapture stack's goal is to make it easy to build applications with different
    // states that require different keyboard shortcuts.
    
    // The easiest example of how KaptureStack can work is something liek View Controllers
    // on iOS. As you create new views (or in this case keyboard listeners) they are pushed
    // onto a stack, once you are done with that view (or keyboard listener) you pop
    // it off the stack and return to your previous state.
    
    // In the web application world it's likely this will be used in one of two different
    // ways.
    
    // 1. A typical stack, as a site brings up something like an 'edit' modal or a alert
    // modal the kapturestack will create a new object, and register new keyboard
    // shortcuts. Once the modal has been removed, the developer calls stack.pop();
    // and the previously registered keyboard shortcuts are once again live.
    // Additionally, "fallthrough" of handlers will make this easier, so a 'root'
    // Kapture handler could manage a large set of keys, and additions to the stack
    // simply place priority to other objects for handling a keypress.
    //
    // 2. A state machine, sometimes a stack isn't the right thing to do, especially
    // if you have some clear states your app will be in. Having some clear named
    // states here should make it easy to move back and forth, without the need to
    // constantly be recreating the handler objects. (if your state changes a ton
    // between 3-4 states this is much better for you)
    ...
    KaptureStack.prototype.key_down = function(event) {
        return this.get_current_handler().key_event(event, "KEYDOWN");
    };

    KaptureStack.prototype.key_up = function(event) {
        return this.get_current_handler().key_event(event, "KEYUP");
    };
    
    // This seems like a dangerous race condition if someone
    // happens to hit a key in this moment, especially if it is
    // a push command.
    KaptureStack.prototype.push = function() {
        this.listener_stack.push( new Kapture() );
        return this.get_current_handler();
    };
    
    // I'm not sure if this is the right behaviour here.
    // not allowing the user to redefine the root handler
    // seems like a bad thing, but I'm struggling to understand
    // when a developer would reasonably do this.
    //   It feels wrong, but I'm leaving the console.log in
    KaptureStack.prototype.pop = function() {
        if (this.listener_stack.length > 1) {
            this.listener_stack.pop();
            return true;
        } else {
            console.log("You removed all the keyboard handlers, which is weird.");
            return false;
        }
    };
