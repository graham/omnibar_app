...
var kapture = (function() {
    ...
    Kapture.prototype.add_active_command = function(key, func, doc) {
    ...
    KaptureStack.prototype.get_root_handler = function() {
	return this.listener_stack[0];
    };
    
    KaptureStack.prototype.key_down = function(event) {
	var len = this.listener_stack.length;
	for(var index=1; index <= len; index++) {
	    var result = this.listener_stack[len-index].key_event(event, "KEYDOWN");
	    if (result == true) {
		return true;
	    }
	}
	...
	var len = this.listener_stack.length;
	for(var index=1; index <= len; index++) {
	    var result = this.listener_stack[len-index].key_event(event, "KEYUP");
	    if (result == true) {
		return true;
	    }
	}
    ...
    KaptureStack.prototype.push = function(kap) {
	if (kap === undefined) {
	    kap = new Kapture();
	}
        this.listener_stack.push(kap);
