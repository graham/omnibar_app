...
var search_chars = [
    "\\", // Exclusive search, exclude matches to this search.
    ...
    for(var i=0; i < s.length; i++) {
        ...
        if (exit_char != null) {
            if ( (open_range_chars.indexOf(chr) != -1) &&
                 (match_chars.indexOf(last_char) != -1) ) {
                ...
                if (eq_hit != -1 && buffer[0] != '=') {
                    entries.push([buffer[0],
                                  ...
                                  buffer.slice(eq_hit, eq_hit+search_hit.length),
                                  buffer.slice(eq_hit+search_hit.length, buffer.length).join('')]);
                } else {
